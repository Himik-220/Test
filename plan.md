...
**Тест №1.4 ZeroDiscriminant (позитивный)**

- **Цель:** Проверка правильности обработки случая с дискриминантом равном нулю, когда корень один.
- **Входные данные:** Коэффициенты уравнения, например, `a = 1`, `b = -4`, `c = -4`.
- **Ожидаемый результат:** 
  - Корни: `x1 = 2` и `x2 = null`.
- **Описание процесса:** Используется стандартный вызов функции с проверкой возвращаемых значений. Проверяется корректность обработки случая, когда дискриминант равен нулю.
